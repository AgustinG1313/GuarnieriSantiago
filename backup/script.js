document.addEventListener('DOMContentLoaded', function () {
    ...
    const modalHandler = {
        ...
        open(artworkId) {
            ...
            // Ya no necesitamos añadir ni quitar .is-opening manualmente
            // porque la animación se maneja con CSS al añadir/quitar .show
        },
        close() {
            // Iniciar la animación de cierre
            modal.classList.remove('show');

            // Limpiar estilos del body cuando termine la animación
            const handleTransitionEnd = () => {
                if (!modal.classList.contains('show')) { // Solo si realmente se cerró
                    document.body.style.overflow = '';
                    document.body.classList.remove('modal-is-open');
                }
                modal.removeEventListener('animationend', handleTransitionEnd); // Limpiar listener
            };

            // Añadir listener para cuando termine la animación de cierre
